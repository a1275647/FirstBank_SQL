...
| 甲方正式機 | 已部署 | 2026-08-28 | 甲方 | 與候選 commit 一致（44617c6b08173363108bf12b5c5e732e6955fdc4） | 使用者於 2026-08-31 確認已部署且驗證完成 |
...
- [x] 已確認三批可獨立執行，無先後相依順序
- [x] 已確認正式機 dbo.EL_ELLSTAPV_D_MF 尚未存在 ELLSTAPV_EXT_DATE 欄位
- [x] 已確認正式機 dbo.DAILY_CIF_TMP 仍存在 PK_DAILY_CIF_TMP 主鍵
- [x] 已確認正式機 dbo.FL_FLMST_D_MF／dbo.FM_FMLINE_D_MF 缺少腳本內列出的欄位
- [x] 已確認正式機 rollback 或異常處理方式（2026082703 未使用 transaction，需另行確認回復方式）
...
- [x] 已確認三份 SQL 均執行成功
- [x] 已確認 dbo.EL_ELLSTAPV_D_MF.ELLSTAPV_EXT_DATE 已存在且型別為 date
- [x] 已確認 dbo.DAILY_CIF_TMP 已無 PK_DAILY_CIF_TMP
- [x] 已確認 dbo.FL_FLMST_D_MF／dbo.FM_FMLINE_D_MF 缺少欄位均已補齊
- [x] 已記錄驗證結果與異常處理
...
| 2026-08-31 | 甲方正式機 | 待補 | 確認正式機驗證結果 | 驗證完成 | 使用者確認；三份 SQL 執行成功、三張表結構符合預期 |
| 2026-08-28 | 甲方正式機 | 甲方 | 執行 2026082701/02/03 Migration | 已完成 | commit 與候選 commit 一致；2026-08-31 使用者補述 |
...
- 正式機驗證已於 2026-08-31 確認完成（三份 SQL 執行成功、三張表結構符合預期）；實際執行日期為 2026-08-28，執行人為甲方，執行的 commit 與候選 commit 一致。
- 兩台測試機的實際 commit、checksum、執行資訊尚未取得，不得直接填入正式機候選版本。
