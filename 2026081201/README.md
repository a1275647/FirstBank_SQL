...
- `CountryMaster.CreditRatingScore`：`tinyint NOT NULL`，預設第五等（5），保存目前國家分數。
- `CountryMaster.CreditRatingScorePublishedAt`：`datetime NULL`，保存國家分數最近一次成功發布時間。

本批次不會：

- 刪除或重新命名舊表 `CreditRating_Country_M`、`CreditRating_Country`。
- 修改 `ufn_table_GetCountryRating`。
- 將歷史資料回填到 `CreditRating_Country_Current`；`CountryMaster.CreditRatingScore` 只套用預設值 5，發布時間不回填。
- 切換 `FirstBank_API` 的讀取來源；該專案仍可繼續讀舊表。

`CreditRating_Country_Current` 不保存 `CreditRating_Country_Log_Detail` 的外鍵；兩者由應用程式在同一發布流程分別寫入，刪除歷程明細時不會被 Current 的外鍵阻擋。
...
4. 首次建立 `CreditRating_Country_Current` 及更新 `CountryMaster` 分數所需的成功條件，由需求負責人決定；本批次 SQL 不代替程式端進行此項業務判定。
...
- 舊明細資料以 `FK_Country_Id = FK_CountryId` 且 `date = CONVERT(date, Create_date)` 的條件尋找歷程主檔；搬入新表後，原 `date` 值寫入 `updated_Date datetime`。
...
- `CountryMaster.CreditRatingScore` 由資料庫預設為 5，`CreditRatingScorePublishedAt` 保持 `NULL`。
...
- 歷史資料回補不得使 `CreditRating_Country_Current.updated_Date` 退回較舊日期。上述發布規則屬於應用程式端的交易邏輯，不由本批次 SQL 自動執行。
