...
BEGIN
    IF COL_LENGTH(N'dbo.CreditRating_Country_Current', N'PK_Id') IS NULL
       ...
       OR COL_LENGTH(N'dbo.CreditRating_Country_Current', N'updated_Date') IS NULL
       ...
       OR (SELECT COUNT(*) FROM sys.columns WHERE [object_id] = OBJECT_ID(N'[dbo].[CreditRating_Country_Current]')) <> 12
    ...
    IF NOT EXISTS
       ...
       (
           ...
           WHERE c.[object_id] = OBJECT_ID(N'[dbo].[CreditRating_Country_Current]')
             AND
             (
                 (c.[name] IN (N'FK_Country_Id', N'FK_RatingAgency_Id') AND (t.[name] <> N'int' OR c.is_nullable <> 0))
